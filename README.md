...
<img width="1024" height="572" alt="Student Management System" src="https://github.com/user-attachments/assets/0032070f-c8eb-4cc9-8449-762bb96e53fd" />

<details open>
<summary>受講生管理システム</summary>
...
受講生を管理できるアプリケーションです。<br>
CRUD処理(Create, Read, Update, Delete)で受講生情報・コース情報の検索、新規登録、詳細編集、論理削除を実装しました。
...
Java・Spring BootをプログラミンスクールのWebアプリケーション開発講座に従って作成したプロジェクトになります。
フロントエンドの実装はオンラインコース、独学、AIガイド付き学習モードを使用しました。
...
→ユーザー側（管理スタッフ）はパソコンの知識があまりなくても、分かりやすく、使いやすいアプリケーションを考えて実装しました。<br>
Java・SpringBootコースを完走した時、フロントエンドに興味もあった結果、最新技術を使用した実装までしてきました。<br><br>

### 1. コースの最終課題に「申込状況」の追加ということで、全体的にリファクタリングし、メソッド名やテストケースを修正しました。<br>
...
### 2.URI Path を REST APIエンドポイントの命名に関するベストプラクティスを考慮しました

→/registerStudent・/updateStudent  から ⇒  /students・/students/{id} に変更。</br>
その後、フロントエンドを実装する時に＠RequestMapping("/api/v1...")を追加。</br>

### 3. クライアント側でユーザビリティを大切にして、自動採番処理を実装してきました。</br>
...
#### 【最大の課題：プログラム設計】</br>
Javaコースを受講中に、アプリケーションのタスク分解を一人で考えることが難しいと思いました。</br>
実際の現場でチーム開発によって、タスク分解がスムーズにできる想像しています。</br>
フロントエンドは特に、一人で設計を考えて、実装をどう進んでいくべきも大変でしたが、最終的に実行できて感動しました。</br>
